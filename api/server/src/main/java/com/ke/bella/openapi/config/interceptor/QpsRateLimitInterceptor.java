...
import com.ke.bella.openapi.domain.protocol.limiter.QpsCheckResult;
import com.ke.bella.openapi.domain.protocol.limiter.QpsLimiterManager;
...
public class QpsRateLimitInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
		...
		if (!result.isAllowed()) {
			...
			throw new OneTokenException.RateLimitException(
				String.format("QPS limit exceeded (current: %d, limit: %d), please retry after 1 second",
