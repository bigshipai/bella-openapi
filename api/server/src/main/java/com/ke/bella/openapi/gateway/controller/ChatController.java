...
import com.ke.bella.openapi.modules.safety.SafetyCheckHelper;
import com.ke.bella.openapi.modules.safety.SafetyCheckRequest;
import com.ke.bella.openapi.modules.channel.ChannelRepo;
...
public class ChatController {
    ...
    public Object completion(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
        ...
        String model = isDirectMode ? OneTokenContext.getDirectModel() : request.getModel();
        ...
        if(model != null && model.contains(",")) {
            ...
            if(lastException != null) {
                throw OneTokenException.fromException(lastException);
            } else {
                throw new BizParamCheckException("None of the specified models can process the request");
            }
        }
    ...
    private void fillMockProperty(CompletionProperty property) {
        Map<String, String> requestInfo = OneTokenContext.getHeaders();
    ...
    private CompletionAdaptor<?> decorateAdaptor(CompletionAdaptor<?> adaptor, CompletionProperty property, EndpointProcessData processData) {
        if(StringUtils.isNotBlank(property.getQueueName())) {
            if(adaptor instanceof CompletionAdaptorDelegator) {
                ...
                throw new IllegalStateException(adaptor.getClass().getSimpleName() + " does not support request proxy");
            }
        }
    ...
    private Object processDirectModeRequest(String endpoint, byte[] bodyBytes, HttpServletResponse httpResponse) throws IOException {
        String model = OneTokenContext.getDirectModel();
